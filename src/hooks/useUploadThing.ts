...
const useUploadThing = () => {
  ...
  //get presigned URLs
  const { mutate: createPresignedUrls, isPending: creatingPresignedUrls } =
    useMutation<{ presignedUrl: string }, Error, FileUploadRequestPayload>({
      mutationFn: async (
        fileUploadRequestPayload: FileUploadRequestPayload
      ) => {
        const response = await UploadThingService.postApiUploadthing(
